...
func (messenger *panelMessenger) getContactRX(msg *message.GetPrintContactMainProcessToRenderer) {
	// The select panel's messenger sent the message to the main process so it will handle the error if there is one.
	if msg.Error {
		return
	}
	messenger.presenter.displayContact(msg.Record)
}
