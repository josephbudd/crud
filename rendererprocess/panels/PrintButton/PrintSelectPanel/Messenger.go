...
// GetPrintSelectContactsPage

// Tell the main process to send back a page of records.
func (messenger *panelMessenger) getPrintSelectContactsPage(firstRecordIndex, max, state uint64) {
	log.Println("getPrintSelectContactsPage()")
...
func (messenger *panelMessenger) getPrintSelectContactsPageRX(msg *message.GetPrintSelectContactsPageMainProcessToRenderer) {
	log.Println("getPrintSelectContactsPageRX()")
	if msg.Error {
		display.Error(msg.ErrorMessage)
		return
	}
...
// Tells the main process to send back a contact record for printing.
...
func (messenger *panelMessenger) getContactRX(msg *message.GetPrintContactMainProcessToRenderer) {
	// This messenger sent the message to the main process so this messenger will handle the error if there is one.
	if msg.Error {
		display.Error(msg.ErrorMessage)
		return
	}
	// No error so let the PrintPrintPanel handle this.
}
			...
			case msg := <-receiveCh:
				...
				*/
				...
				case *message.GetPrintSelectContactsPageMainProcessToRenderer:
					messenger.getPrintSelectContactsPageRX(msg)
