...
func (controler *panelControler) defineControlsSetHandlers() (err error) {
	...
	*/
	...
	if controler.removeSelectWidgetWrapper = notJS.GetElementByID("removeSelectWidgetWrapper"); controler.removeSelectWidgetWrapper == null {
	...
	controler.vlist = kickwasmwidgets.NewVList(controler.removeSelectWidgetWrapper,
		vlistID,
		...
		func() {
			controler.group.showRemoveNotReadyPanel(false)
		},
		// needToInitializeFunc func(count, state uint64),
		func(count, state uint64) {
			controler.caller.getRemoveSelectContactsPage(0, count, state)
		},
		...
		func(button js.Value, count, state uint64) {
			...
			controler.caller.getRemoveSelectContactsPage(startIndex, actualCount, state)
		},
		...
		func(button js.Value, count, state uint64) {
			buttonIndex := notJS.GetAttributeUint64(button, controler.sortedIndexAttributeName)
			controler.caller.getRemoveSelectContactsPage(buttonIndex, count, state)
		},
		notJS,
		tools,
	)
