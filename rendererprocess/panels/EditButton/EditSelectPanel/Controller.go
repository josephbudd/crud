...
type panelController struct {
	...
	*/
	...
	editSelectWidgetWrapper *markup.Element
...
func (controller *panelController) defineControlsHandlers() (err error) {
	...
	*/
	...
	if controller.editSelectWidgetWrapper = document.ElementByID("editSelectWidgetWrapper"); controller.editSelectWidgetWrapper == nil {
		err = fmt.Errorf("unable to find #editSelectWidgetWrapper")
		return
	}
	// Define the vlist.
	...
	controller.vlist = kickwasmwidgets.NewVList(controller.editSelectWidgetWrapper,
		...
		func(count, state uint64) {
			controller.messenger.getEditSelectContactsPage(0, count, state)
		},
		...
		func(button *markup.Element, count, state uint64) {
			...
			controller.messenger.getEditSelectContactsPage(startIndex, actualCount, state)
		},
		...
		func(button *markup.Element, count, state uint64) {
			buttonIndex, _ := button.AttributeUint64(sortedIndexAttributeName)
			controller.messenger.getEditSelectContactsPage(buttonIndex, count, state)
		},
		document,
	)
...
//   which calls the messenger's getContact func so that a contact record can be edited.
