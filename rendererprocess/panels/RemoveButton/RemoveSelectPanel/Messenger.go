...
func (messenger *panelMessenger) getRemoveSelectContactsPage(firstRecordIndex, max, state uint64) {
...
func (messenger *panelMessenger) getRemoveSelectContactsPageRX(msg *message.GetRemoveSelectContactsPageMainProcessToRenderer) {
	if msg.Error {
		display.Error(msg.ErrorMessage)
		return
	}
...
func (messenger *panelMessenger) getContactRX(msg *message.GetRemoveContactMainProcessToRenderer) {
			...
			case msg := <-receiveCh:
				...
				*/
				...
				case *message.GetRemoveSelectContactsPageMainProcessToRenderer:
					messenger.getRemoveSelectContactsPageRX(msg)
